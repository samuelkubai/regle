...
VARIABLES=(
  ...
  'IMAGE_TAG' 'REACT_APP_JWT_PUBLIC_KEY'
)
...
findTempateFiles() {
  ...
  local _templates=$(find $ROOT_DIRECTORY/scripts/deploy -name "*.tpl" -type f)
...
findAndReplaceVariables() {
  ...
  for file in ${TEMPLATES[@]}; do
    ...
    then
      ...
      if [[ $? == 0 ]]; then
          success "Template file $(basename $file) has been successfully built to $(basename $output)"
        else
